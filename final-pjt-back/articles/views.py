...
def like_comment(request, article_pk, comment_pk):
    ...
    user = request.user
    if comment.user_liked.filter(pk = user.pk).exists():
        comment.user_liked.remove(user)
        serializer = CommentSerializer(comment)
        return Response(serializer.data)
    else:
        comment.user_liked.add(user)
        serializer = CommentSerializer(comment)
        return Response(serializer.data)
    

@api_view(['POST'])
def like_comment_len (request, article_pk, comment_pk):
    comment = get_object_or_404(Comment, pk=comment_pk)
    user = request.user
    serializer = CommentSerializer(comment)
    return Response(serializer.data)
